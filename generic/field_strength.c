...
void make_field_strength(  field_offset link_src,       /* field offset for su3_matrix[4] type 
  ...
  )
{
  ...
  for(component=FS_XY;component<=FS_ZT;component++)
	...
	case FS_ZT: dir0=ZUP; dir1=TUP; break;
      ...
      mtag0 = start_gather_site( LINK_OFFSET(dir0), sizeof(su3_matrix), dir1, EVENANDODD, gen_pt[0] );
      mtag1 = start_gather_site( LINK_OFFSET(dir1), sizeof(su3_matrix), dir0, EVENANDODD, gen_pt[1] );
      
      wait_gather(mtag0);
      wait_gather(mtag1);
      FORALLSITES(i,s)
	{
	  mult_su3_nn( &LINK(dir0), (su3_matrix *)(gen_pt[1][i]), &tmat1 );
      ...
      printf("\n Amit: generic/field_strength.c Amitcounter=%i \n",AmitCounter);
      AmitCounter=0.0;
      ...
      //  Plaquette in -dir0 +dir1 direction 
      //Comment //// mtag0 = start_gather_site( LINK_OFFSET(dir0), 
      // //sizeof(su3_matrix), dir1, EVENANDODD, gen_pt[0] );
      // //wait_gather(mtag0);  Already gathered above/// Comment end here
      ...
      FORALLSITES(i,s)
	{
      ...
      mtag1 = start_gather_field( temp1, sizeof(su3_matrix), OPP_DIR(dir0), EVENANDODD, gen_pt[1] );
      wait_gather(mtag1);
      FORALLSITES(i,s)
	{
	  mult_su3_nn( &LINK(dir1), (su3_matrix *)(gen_pt[1][i]), &tmat1 );
	  su3_adjoint( &tmat1, &tmat2 );
	  add_su3_matrix( &FIELD_STRENGTH(component), &tmat1, &FIELD_STRENGTH(component) );
	  sub_su3_matrix( &FIELD_STRENGTH(component), &tmat2, &FIELD_STRENGTH(component) );
	  AmitCounter++;
	}
      printf("\n Amit: generic/field_strength.c Amitcounter=%i \n",AmitCounter);
      AmitCounter=0.0;
      ...
      // Plaquette in -dir0 -dir1 direction 
    mtag0 = start_gather_site( LINK_OFFSET(dir0), sizeof(su3_matrix), OPP_DIR(dir0), EVENANDODD, gen_pt[0] );
    mtag1 = start_gather_site( LINK_OFFSET(dir1), sizeof(su3_matrix),   OPP_DIR(dir1), EVENANDODD, gen_pt[1] );
    wait_gather(mtag0);
    wait_gather(mtag1);
    FORALLSITES(i,s)
      {
    ...
    cleanup_gather(mtag0);
    cleanup_gather(mtag1);
    mtag0 = start_gather_field( temp1, sizeof(su3_matrix), OPP_DIR(dir1), EVENANDODD, gen_pt[0] );
    wait_gather(mtag0);
    mtag1 = start_gather_field( temp2, sizeof(su3_matrix), OPP_DIR(dir0), EVENANDODD, gen_pt[1] );
    wait_gather(mtag1);
    FORALLSITES(i,s)
      {
	mult_su3_an( (su3_matrix *)(gen_pt[1][i]), (su3_matrix *)(gen_pt[0][i]), &tmat1 );
	su3_adjoint( &tmat1, &tmat2 );
	add_su3_matrix( &FIELD_STRENGTH(component), &tmat1, &FIELD_STRENGTH(component) );
	sub_su3_matrix( &FIELD_STRENGTH(component), &tmat2, &FIELD_STRENGTH(component) );
	AmitCounter++;
